...
- [x] Video plays INLINE (no external player): pure-Rust `yscv-video` (`src/video.rs` `VideoStore`, mirroring `MermaidStore`) decodes MP4 H.264/HEVC off-thread into `RenderImage` frames — RGB8→RGBA + atlas BGRA swap like `SvgRenderer`, downscaled to 640px, capped at 300 frames. Play/Pause via a frame-pacing timer loop (`background_executor().timer`, Zed blink-manager pattern) with a generation guard; click-to-seek scrub bar (canvas bounds tracker maps window-space clicks to frames — no window needed in the render chain), -1s/+1s step, time label, silent-preview note; remote URLs download via `curl` to temp then decode the same path; errors fall back to Open/external. Proven by `decode_real_mp4` (real 30-frame clip decodes in 0.14s; zero-timestamp containers get synthesized 30fps pacing).
