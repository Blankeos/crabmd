// Generate crabmd's built-in theme set from OpenCode's TUI themes.
// Run via: `bun run scripts/gen-themes.ts`
// Ported from ../crabcode/scripts/gen-themes.ts — same upstream source,
// adapted to crabmd's `themes/` dir + `THEME_FILES` in src/theme.rs.
...
// Hand-authored desktop-schema themes (`light`/`dark` seeds+overrides).
// Never deleted, never fetched — kept as-is and merged into THEME_FILES.
const HAND_AUTHORED = ['crabcode-orange', 'grokday', 'groknight']
 ...
 * "transparent" or semi-transparent `#rrggbbaa`; replace / strip so the GUI
 * paints a real opaque bg.
 */
 ...
 * luminance. Searchable in the theme picker.
 */
 ...
 * with `appearance: "light"` so the picker can filter/select the light palette
...
function makeLightSibling(
  ...
  if (typeof sibling.name === 'string' && sibling.name.length > 0) {
    ...
    // Title-case the id for nicer picker labels (e.g. "Github Light").
    sibling.name =
      baseId
        .split('-')
...
function writeThemeFilesList(allIds: string[]) {
  const entries = allIds
    .map((id) => `    ("${id}", include_str!("../themes/${id}.json")),`)
    .join('\n')

  const themeRsPath = join(process.cwd(), 'src', 'theme.rs')
  const themeRs = readFileSync(themeRsPath, 'utf8')
  const start = 'pub const THEME_FILES: &[(&str, &str)] = &['
  const end = '];'
  const startIdx = themeRs.indexOf(start)
  if (startIdx < 0) throw new Error('THEME_FILES start marker not found in src/theme.rs')
  const endIdx = themeRs.indexOf(end, startIdx)
  if (endIdx < 0) throw new Error('THEME_FILES end marker not found in src/theme.rs')
  ...
  console.log(`Updated THEME_FILES in src/theme.rs (${allIds.length} themes)`)
}
...
async function fetchThemes() {
  ...
  // Clear previously generated files, keep hand-authored ones.
  for (const entry of readdirSync(THEMES_DIR)) {
    if (!entry.endsWith('.json')) continue
    const id = entry.replace(/\.json$/, '')
    if (HAND_AUTHORED.includes(id)) continue
    rmSync(join(THEMES_DIR, entry))
  }
  ...
  const allIds = [...HAND_AUTHORED, ...generatedIds].sort((a, b) => a.localeCompare(b))
  writeThemeFilesList(allIds)
