#!/usr/bin/env python3
"""Compare crabmd vs peer markdown editors on startup + idle CPU.
...
Note: crabmd is a GPUI GUI — section B (PTY first-frame) is best-effort;
prefer section A (--help) + section C idle with --section version,idle.
"""
...
DEFAULT_AGENTS = ("crabmd", "typora")

# argv to launch interactive TUI (no prompt / no print mode)
AGENT_ARGV: dict[str, list[str]] = {
    "crabmd": ["crabmd", "-w", "examples/kitchen-sink.md"],
    "typora": ["typora", "examples/kitchen-sink.md"],
}
